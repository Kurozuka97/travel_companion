  ...
  async function handleSearch() {
    ...
    try {
      ...
      if (data.places?.length === 0) {
        setError(`No places found. Source: ${data.source}. Try searching for "nasi lemak", "langkawi", "penang", "kl", "melaka", or "cameron" for instant results.`);
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      <div className="bg-slate-800 border-b border-slate-700">
        <div className="max-w-6xl mx-auto px-4 py-6">
          ...
          <p className="text-slate-400 mt-1">Discover hidden gems with AI-powered search</p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex gap-2">
            <input
              ...
              placeholder="e.g. best nasi lemak in KL, hiking trails Penang..."
        ...
        {error && (
          <div className="bg-amber-900/50 border border-amber-700 rounded-lg p-4 mb-6 text-amber-300">
              ...
              <div>
                ...
                {debug && (
                  <div className="mt-2 text-xs text-amber-400/70 font-mono">
                    <p>API Keys: Groq={debug.hasGroqKey ? '✓' : '✗'} OpenRouter={debug.hasOpenRouterKey ? '✓' : '✗'} Mistral={debug.hasMistralKey ? '✓' : '✗'}</p>
                    <p>Errors: {debug.errors?.join(', ')}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {filteredPlaces.length > 0 ? (
            ...
            {filteredPlaces.map((place, i) => (
              <div key={i} className="bg-slate-800 rounded-xl border border-slate-700 p-5 hover:border-slate-600 transition group">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`px-3 py-1 text-xs rounded-full text-white font-medium ${categoryColors[place.category?.toLowerCase()] || categoryColors.general}`}>
                ...
                <p className="text-slate-400 text-sm mb-3 flex items-center gap-2">
                  ...
                  <span className="line-clamp-1">{place.address || 'Malaysia'}</span>
                </p>
                ...
                <a 
                  href={`https://www.google.com/maps/search/${encodeURIComponent(place.name + ' ' + (place.address || 'Malaysia'))}`}
                  target="_blank"
          ...
          !loading && !error && (
            <div className="text-center py-20 text-slate-500">
              ...
              <p className="text-sm mt-2">Try "nasi lemak", "langkawi", "penang", "kl", "melaka", or "cameron"</p>
            </div>
          )
        )}
      </div>
